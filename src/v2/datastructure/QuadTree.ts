...
export class QuadTree implements Iterable<QuadTree> {
    ...
    public contains(x: number, y: number) {
        ...
        return (this.cell.getMinX() <= x && this.cell.getMinY() <= y &&
     ...
     */
    public getGlyphs(): ArrayList<Glyph> {
        return this.glyphs;
    }
    ...
    getLeaves(...args: [Side] | [Side, ArrayList<QuadTree>] | [Side, Rectangle2D, ArrayList<QuadTree>] |
        [Rectangle2D] | [Rectangle2D, ArrayList<QuadTree>] |
        [Glyph, number] | [Glyph, number, ArrayList<QuadTree>]){
    ...
    __getLeavesRectangle(rectangle: Rectangle2D): ArrayList<QuadTree> {
        ...
        const result = new ArrayList<QuadTree>();
        this.getLeaves(rectangle, result);
    ...
    __getLeavesGlyphAt(glyph: Glyph, at: number): ArrayList<QuadTree> {
        Timers.start("[QuadTree] getLeaves");
        const result = new ArrayList<QuadTree>();
        this.getLeaves(glyph, at, result);
        Timers.stop("[QuadTree] getLeaves");
        return result;
    }
    ...
    toString(): string {
        return `${QuadTree.name}[cell = [${this.cell.getMinX()} ; ${this.cell.getMaxX()}] x [${this.cell.getMaxX()} ; ${this.cell.getMaxY()}]]`;
     ...
     */
    private __getLeavesGlyphAtResult(glyph: Glyph, at: number, result: ArrayList<QuadTree>) {
        ...
        if (this.isLeaf()) {
            ...
            for (const child of this.children) {
                child.getLeaves(glyph, at, result);
            }
        }
    }
    ...
    __getLeavesSideResult(side: Side, result: ArrayList<QuadTree>) {
        this.getLeaves(side, null, result);
    }
     ...
     * @param result    Set to add cells to.
     */
    __getLeavesRectangleResult(rectangle: Rectangle2D, result: ArrayList<QuadTree>) {
        if (Utils.intervalsOverlap([this.cell.getMinX(), this.cell.getMaxX()],
            [rectangle.getMinX(), rectangle.getMaxX()]) &&
            Utils.intervalsOverlap([this.cell.getMinY(), this.cell.getMaxY()],
     ...
     * @see QuadTree#getLeaves(Side, ArrayList)
     */
    __getLeavesSideRectangleResult(side: Side, range: Rectangle2D, result: ArrayList<QuadTree>) {
        if (range != null) {
            ...
            const c: [number, number] = [null, null];
        ...
        for (const i of side.quadrants()) {
            this.children[i].getLeaves(side, range, result);
        }
    }
                ...
                this.children[quadrant].neighbors.get(side.ordinal()).addAll(
                    neighborsOnSide.stream()
                        ...
                        .filter((neighbor) => Utils.openIntervalsOverlap(qi,
                            Side.interval(neighbor.cell,
                                side)))
                        .collect(Collectors.toList())
                );
                // the children are neighbors of each other; record this
                this.children[quadrant].neighbors.get(side.opposite().ordinal()).add(
