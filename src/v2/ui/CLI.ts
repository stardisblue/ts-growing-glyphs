...
export class CLI {
    static async main(args: string[]) {
        ...
        try {
            ConfigurableConsoleHandler.redirectTo(new PrintStream(new File("non_regression.txt")));
        } catch (e) {
        ...
        try {
            ...
            if (LOGGER != null) {
                ...
                LOGGER.log(Level.FINE, `${(Constants.STATS_ENABLED ? "" : "not ")}collecting stats`,
                );
                LOGGER.log(Level.FINE, `${Constants.ROBUST ? "" : "not "}using QUAD+ optimization`,
                );
                LOGGER.log(Level.FINE, `${Constants.BIG_GLYPHS ? "" : "not "}using big glyph optimization`,
                );
            }
            clusterer.cluster();
        } catch (e) {
