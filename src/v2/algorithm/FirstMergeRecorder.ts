...
function getLogger() {
    ...
    return Constants.LOGGING_ENABLED && (l = Logger.getLogger("FirstMergeRecorder")).isLoggable(Level.FINER) ? l : null;
}
...
export class FirstMerge {
    ...
    constructor() {
        this.at = ArrayList.__new(Arrays.nCopies(
            Constants.MAX_MERGES_TO_RECORD, Number.POSITIVE_INFINITY));
    ...
    public accept(parent: FirstMergeRecorder, candidate: Glyph): void {
        ...
        const at = GrowFunction.intersectAt(parent._from, candidate);
        for (let i = 0; i < Constants.MAX_MERGES_TO_RECORD; ++i) {
            if (at < this.at.get(i)) {
        ...
        if (LOGGER != null) {
            LOGGER.log(Level.FINER, `#${this} now h as glyphs ${this.glyphs.toArray()} at ${this.at.toArray()}`);
...
#${that} has glyphs ${that.glyphs.toArray()} at ${that.at.toArray()}`);
        ...
        if (LOGGER != null) {
            LOGGER.log(Level.FINER, `result #${result} of merging #${this} and ${that
            } has glyphs ${result.glyphs.toArray()} at ${result.at.toArray()} (storing in #${this} now)`);            // LOGGER.log(Level.FINER,
    ...
    pop(parent: FirstMergeRecorder): GlyphMerge[] {
        if (this.size === 0) {
            return null;
        }
        ...
        for (const wth of glyphs) {
            result[i++] = new GlyphMerge(parent._from, wth,
...
export class FirstMergeRecorder {
    ...
    private static firstReusedRecord: FirstMerge = null;
     ...
     */
    public addEventsTo(q: MultiQueue, l: Logger): void
    ...
    public record(glyphs: Glyph[] | Stream<Glyph> | ArrayList<Glyph>, from?: number, upto?: number): void {
     ...
     * @param upto   Index up to but excluding which glyphs will be recorded.
     */
    public __recordGlyphNumberNumber(glyphs: Glyph[], from: number, upto: number): void {
    ...
    public __recordStream(glyphs: Stream<Glyph>) {
        if (Constants.ROBUST) {
            this.merge.getGlyphs().addAll(glyphs.parallel()
                .filter((glyph) => glyph.isAlive() && glyph !== this._from)
                .collect(Collectors.toSet()));
        } else {
            this.merge.combine(glyphs
                .filter((glyph) => glyph.isAlive() && glyph !== this._from)
