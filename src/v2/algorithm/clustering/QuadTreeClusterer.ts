...
export class QuadTreeClusterer {
    ...
    cluster(): QuadTreeClusterer {
        ...
        const map = new HashMap<Glyph, HierarchicalClustering>(); // TODO
     ...
     */
    private findOverlap(wth: Glyph, at: number,
                        addTo: PriorityQueue<GlyphMerge>, bigGlyphs: ArrayList<Glyph>): boolean {
...
// check the queue
    ...
    private handleBigGlyphMerge(m: GlyphMerge, s: GlobalState, q: MultiQueue, track: boolean) {
        if (Constants.TIMERS_ENABLED) {
    ...
    private handleGlyphMerge(m: GlyphMerge,
                             s: GlobalState, q: MultiQueue, track: boolean) {
        // process the merge and all merges that it causes
        const merged = this.processNestedMerges(m, s, q, track);
    ...
    private handleOutOfCell(o: OutOfCell, map:
        HashMap<Glyph, HierarchicalClustering>, includeOutOfCell: boolean, q:
                                MultiQueue): void {
