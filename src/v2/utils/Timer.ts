...
export class Timer {
    ...
    public log(logger: Logger, name: string, level: Level = Level.FINE): void {
        stop();
        if (logger !== null) {
            logger.log(level, `${name} took ${
                Timers.in(this.totalElapsed, Units.SECONDS).toFixed(2).padStart(5)
            } seconds (wall clock time${this.count === 1 ? "" : `, ${this.count} timings`}`);
            ...
            System.out.println(`${name} took ${
                Timers.in(this.totalElapsed, Units.SECONDS).toFixed(3).padStart(5)
            } seconds (wall clock time${this.count === 1 ? "" : `, ${this.count} timings`}`);
