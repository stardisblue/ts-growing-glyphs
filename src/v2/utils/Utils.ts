...
export class Utils {
    ...
    static euclidean(...args: [number, number, number, number] | [Rectangle2D, number, number]): number {
        ...
        } else if (args.length === 3 && isRectangle2D(args[0])
    ...
    static __euclideanRectangleXY(rect: Rectangle2D, px: number, py: number): number {
        ...
        return this.euclidean(px, py, this.clamp(px, rect.getMinX(), rect.getMaxX()),
     ...
     */
    static swap<T>(listA: ArrayList<T>, indexA: number, listB: ArrayList<T>, indexB: number) {
...
export class Stats {
    private static readonly TAG_REGEX: Pattern = Pattern.compile(/^\[([a-z]+)]\\s+/);
    ...
    static logAll(logger: Logger) {
        ...
        const padTo = this.stats.keySet().stream()
            .filter((n) => {
                const tagMatcher = this.TAG_REGEX.matcher(n);
                return !tagMatcher.find() ||
                    tagMatcher.group(1) !== ("perc");
            })
            .map((n) => this.TAG_REGEX.matcher(n).replaceAll(""))
            .max(Comparator.comparingInt(String__length)).get().length;
        // const f = "%1$-" + padTo + "s";
...
export class Timers {
    ...
    static logAll(logger: Logger) {
        ...
        const padTo = this.timers.keySet().stream().max(
            Comparator.comparingInt(String__length)).get().length;
        // const f = "%1$-" + padTo + "s";
        const f = (k: string) => `${k}-${padTo}s`;
        // log entries without section
        this.timers.entrySet().stream()
            .filter((e) => !e.getKey().startsWith("["))
            .sorted(Entry.comparingByKey())
            .forEach((e) =>
                e.getValue().log(logger, f(e.getKey())));
        // log entries in a section
        const timersInSections = this.timers.entrySet().stream()
            .filter((e) => e.getKey().startsWith("["))
            .sorted(Entry.comparingByKey())
            .toArray();
        let lastSection = "";
