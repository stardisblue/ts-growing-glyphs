...
export class Stat {
    ...
    log(logger: Logger, name: string): void {
        if (this.min === this.max) {
            logger.log(Level.FINE, `${name} was ${
                this.max.toFixed(2)} and did not change over ${
                this.n} measurement${
                this.n === 1 ? "" : "s"}`
            )
            ;
            ...
            logger.log(Level.FINE, `${name} was ${
                this.average.toFixed(2)} on average and always between ${
                this.min} and ${
                this.max} over ${
                this.n} measurement${
                this.n === 1 ? "" : "s"}`);
    ...
    logCount(logger: Logger, name: string): void {
        logger.log(Level.FINE, `${name} occured ${
            this.n === 1 ? "once" : this.n + " times"}`);
    }

    logPercentage(logger: Logger, name: string): void {
        logger.log(Level.FINE, `${name} ${
            (this.average * 100).toFixed(2)}% of the time`);
    }
    ...
    unrecord(value: number): void {
        if (this.n === 0) {
            throw new Error("cannot unrecord a value when no "
                + "values are recorded");
        }
        this.average -= (value - this.average) / --this.n;
        this.sum -= value;
