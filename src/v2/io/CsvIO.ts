...
export class CsvIO {
    private static readonly LOGGER: Logger = (Constants.LOGGING_ENABLED ?
        Logger.getLogger(CsvIO.name) : null);
    ...
    public static async read(file: File, tree: QuadTree) {
        ...
        try {
            ...
            while (reader.hasNextLine()) {
                ...
                if (cols.length <= Math.max(latInd, lngInd) || cols[latInd] === ("NULL") ||
                    cols[lngInd] === ("NULL")) {
                    ignoredRead[0]++;
                    continue;
        ...
        if (Constants.LOGGING_ENABLED) {
            CsvIO.LOGGER.log(Level.INFO, `read ${ignoredRead[1]} entries and ignored ${ignoredRead[0]}`);
        }
        if (Constants.LOGGING_ENABLED)
