...
export class CsvIO {
  ...
  static read(file: GGFile, tree: QuadTree) {
    ...
    rl.on('line', (line) => {
      ...
      if (read.has(p.valueOf())) {
        read.set(p.valueOf(), { ...read.get(p.valueOf())!, weight });
      } else {
        read.set(p.valueOf(), { p, weight });
      }
    ...
    for (const values of read.values()) {
      ...
      tree.insertCenterOf(
        new Glyph(p.getX() - 256, p.getY() - 256, values.weight, true)
      );
    }
